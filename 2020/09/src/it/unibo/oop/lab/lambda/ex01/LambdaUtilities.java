...
/**
 * This class will contain four utility functions on lists and maps, of which the first 
 * one is provided as example.
 * All such methods take as second argument a functional interface from the 
 * Java library (java.util.function).
...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
        ...
        /*
         * Optional.ofNullable ritorna il valore o Empty Optional, quindi in returnList
         * ci vanno anche probabili elementi nulli che soddisfano il predicato
         * */
        list.forEach(i -> returnList.add(Optional.ofNullable(i).filter(pre)));
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        ...
        list.forEach(i -> {
            map.merge(op.apply(i), new HashSet<>(Arrays.asList(i)), (i1, i2) -> {
    ...
    public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
        ...
        map.forEach((k, v) -> filledMap.put(k, v.orElse(def.get())));
