...
class WorkWithArrays {
    ...
    static int[] evenElements(final int[] array) {
        final int newLenght = array.length % 2 == 0 ? array.length / 2 : array.length / 2 + 1;
        final int[] returnValue = new int[newLenght];
        for (int i = 0; i < array.length; i += 2) {
            returnValue[i / 2] = array[i];
        }
        return returnValue;
    }

    static int[] oddElements(final int[] array) {
        final int newLenght = array.length / 2;
        final int[] returnValue = new int[newLenght];
    ...
    static int mostRecurringElement(final int[] array) {
        ...
        // Sort will help in the analysis.
        Arrays.sort(copy);
    ...
    static boolean testCountOccurrencies() {
        return countOccurrencies(new int[] { 1, 2, 3, 4 }, 1) == 1
                ...
                && countOccurrencies(new int[] { 1, 2, 2, 2, 4, 1 }, 2) == 3
                && countOccurrencies(new int[] { 5, 5, 5, 5, 5, 5 }, 5) == 6;
    }
